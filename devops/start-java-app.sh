...
JMX_EXPORTER_CONFIG="${JMX_EXPORTER_CONFIG:-/app/jmx_prometheus_config.yml}"
...
start_java_app() {
    ...
    log "JMX Exporter port: $JMX_EXPORTER_PORT"
    ...
    # Check if JMX Exporter exists
    if [ ! -f "/app/jmx_prometheus_javaagent.jar" ]; then
        log_warning "JMX Exporter not found, starting without monitoring"
        JMX_AGENT_OPTS=""
    else
        log "JMX Exporter found, enabling monitoring on port $JMX_EXPORTER_PORT"
        JMX_AGENT_OPTS="-javaagent:/app/jmx_prometheus_javaagent.jar=$JMX_EXPORTER_PORT:$JMX_EXPORTER_CONFIG"
    fi
    
    # Wait for dependencies
    wait_for_dependencies
    
    # Start the application with JMX Exporter
    log "Starting Java application with JMX monitoring..."
    exec java $JAVA_OPTS $JMX_AGENT_OPTS -jar "$JAR_FILE" "$@"
}
